...
public:
	vector<BoolExprTree<ArrN>*> _exprTrees;
	...
	BoolExprTreeManager(std::array<size_t, ArrN>& tags) : _tags(tags) {

	}

	~BoolExprTreeManager() {
		for (BoolExprTree<ArrN>* t : _exprTrees) delete t;
	}

	void DEBUG_Output(BoolExprBitVector<ArrN>& beb) {
		for (size_t i = 0; i < ArrN; ++i) {
			PRINT(", " << (beb._CaresAbout.at(i) & beb._mustHave.at(i)));
		}
		PRINT(std::endl);
	}
	...
	*/
	...
	// If we can add as implicit, MORE COMPLICATED, MORE COMPLEX USE AND MANAGEMENT!!!!!
		...
		BoolExprBitVector<ArrN> newGroupExpr;
		MakeFromSpecVec(has, hasNot, newGroupExpr);
		DEBUG_Output(newGroupExpr);
		...
		if (treesWithExplComponents.size() == 0) {
			PRINT("Making new TREE" << std::endl);
			_exprTrees.push_back(new BoolExprTree<ArrN>());
		...
		// If there is a single tree which can use ALL of its components
			...
			BoolExprTree<ArrN>* tree = _exprTrees[*treesWithExplComponents.begin()];
		...
		// If there are multiple trees which use its components OR a single tree which uses only SOME
			...
			if (addNonImplicitlyIfImplicit) {
				for (size_t t : treesWithExplComponents) {
					...
					BoolExprTree<ArrN>* tree = _exprTrees[t];
			...
			if (createNewTreeIfImplicit && componentsWithoutTrees.size() > 0) {
				BoolExprBitVector<ArrN> newTreeRootExpr;
				MakeFromSpecVec(componentsWithoutTrees, {}, newTreeRootExpr);
